...
func (c *Conn) GetInventory(ctx context.Context) (device *devices.Device, err error) {
	...
	if err != nil {
		return nil, errors.Wrap(ErrRedfishSoftwareInventory, err.Error())
	}
...
func (i *inventory) bmcAttributes(device *devices.Device) (err error) {
	...
	if compatible == 0 {
		return ErrRedfishManagerOdataID
	}
...
func (i *inventory) chassisAttributes(device *devices.Device) (err error) {
	...
	if compatible == 0 {
		return ErrRedfishChassisOdataID
	}
...
func (i *inventory) systemAttributes(device *devices.Device) (err error) {
	...
	for _, sys := range systems {
		...
		i.collectCPUs(sys, device)
		i.collectDIMMs(sys, device)
		i.collectDrives(sys, device)
		i.collectBIOS(sys, device)
		i.collectNICs(sys, device)
		i.collectTPMs(sys, device)
		i.collectStorageControllers(sys, device)
	}

	if compatible == 0 {
		return ErrRedfishSystemOdataID
	}
...
func (i *inventory) collectEnclosure(ch *redfish.Chassis, device *devices.Device) (err error) {
	if ch.Manufacturer != "" && ch.Model != "" && ch.SerialNumber != "" {
		device.Vendor = ch.Manufacturer
		device.Model = ch.Model
		device.Serial = ch.SerialNumber
	}
